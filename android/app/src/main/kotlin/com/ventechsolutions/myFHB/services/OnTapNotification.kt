...
class OnTapNotification:BroadcastReceiver() {
    ...
    override fun onReceive(p0: Context?, p1: Intent?) {
        ...
        val sender_id = p1?.getStringExtra(p0?.getString(R.string.senderId))
        val sender_name = p1?.getStringExtra(p0?.getString(R.string.senderName))
        val sender_profile = p1?.getStringExtra(p0?.getString(R.string.senderProfilePic))
        val notificationListId = p1?.getStringExtra(p0?.getString(R.string.notificationListId))
        val group_id = p1?.getStringExtra(p0?.getString(R.string.chatListId))
        ...
        launchIntent?.putExtra(Constants.PROP_DATA,data)
                launchIntent?.putExtra(Constants.PROP_sheelaAudioMsgUrl,audioURL)

        launchIntent?.putExtra(Constants.PROP_EVEID,EVEId)
        launchIntent?.putExtra(p0.getString(R.string.docId), docId)
        launchIntent?.putExtra(p0.getString(R.string.senderId), sender_id)
        launchIntent?.putExtra(p0.getString(R.string.senderName), sender_name)
        launchIntent?.putExtra(p0.getString(R.string.senderProfilePic), sender_profile)
        launchIntent?.putExtra(p0.getString(R.string.chatListId), group_id)
