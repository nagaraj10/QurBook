...
//    List<Sharedbyme> profilesSharedByMe = new List();
  ...
  Widget getCardWidgetForUser(
    ...
    return InkWell(
      onTap: () {
        if (position != 0) {
          Navigator.pushNamed(context, Constants.Family_Detail_Route,
                              ...
                              new FHBUtils().check().then((intenet) {
                                if (intenet != null && intenet) {
                                  ...
                                  CommonUtil.showLoadingDialog(context,
                                      _keyLoader, Constants.Please_Wait);

                                  var deLinkingData = {};
                                  deLinkingData['relatedTo'] =
                                      data.profileData.id;
                                  deLinkingData['relationshipType'] =
                                      Constants.parentToChild;
                                  var jsonString =
...
//            ),
  ...
  void _sendOTPBtnTapped() {
    if (mobileNoController.text.length > 0 &&
          ...
          if (isPrimaryNoSelected) {
            ...
            _familyListBloc.postUserLinking(jsonString).then((userLinking) {
              if (userLinking.success && userLinking.status == 200) {
                ...
                Alert.displayAlertPlain(context,
                    title: Constants.Error, content: userLinking.message);
              }
            });
          }
        } else {
      ...
      Alert.displayAlertPlain(context,
          title: Constants.Error, content: CommonConstants.all_fields);
    }
  }
