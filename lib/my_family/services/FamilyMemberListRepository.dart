...
class FamilyMemberListRepository {
  ...
  Future<dynamic> getUserProfile(String doctorsId) async {
    final response = await _helper
        .getDoctorProfilePic(query.qr_doctors + doctorsId + query.qr_profilePic);
    return response;
  }
  ...
  Future<UserLinkingResponseList> postUserLinking(String jsonString) async {
    final response = await _helper.addUserLinking(query.qr_userlinking, jsonString);
  ...
  Future<UserDeLinkingResponseList> postUserDeLinking(String jsonString) async {
    final response =
        await _helper.addUserDeLinking(webserviceCall.getQueryForPostUserDelinking(), jsonString);
