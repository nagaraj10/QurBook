...
class _SelectedDeviceState extends State<SelectedDevice> {
  ...
  Widget projectWidget(BuildContext context) {
    ...
    for (int i = 0; i < widget.deviceValues.length; i++) {
      if (widget.deviceData.value_name == 'bloodPressure') {
        date =
            widget.deviceValues[0].bloodPressure.entities[0].lastsyncdatetime;
        devicevalue1 =
            widget.deviceValues[0].bloodPressure.entities[0].systolic;
        devicevalue2 =
            widget.deviceValues[0].bloodPressure.entities[0].diostolic;
        return getDeviceData(date, devicevalue1, devicevalue2);
      } else if (widget.deviceData.value_name == 'bloodGlucose') {
        date = widget.deviceValues[1].bloodGlucose.entities[0].lastsyncdatetime;
        /* devicevalue1 =
            widget.deviceValues[1].bloodGlucose.entities[0].bloodGlucoseLevel;*/
        return getDeviceData(date, devicevalue1, '');
      } else if (widget.deviceData.value_name == 'oxygenSaturation') {
        date = widget
            .deviceValues[2].oxygenSaturation.entities[0].lastsyncdatetime;
        devicevalue1 = widget
            .deviceValues[2].oxygenSaturation.entities[0].oxygenSaturation;
        return getDeviceData(date, devicevalue1, '');
      } else if (widget.deviceData.value_name == 'bodyTemperature') {
        date =
            widget.deviceValues[3].bodyTemperature.entities[0].lastsyncdatetime;
        devicevalue1 =
            widget.deviceValues[3].bodyTemperature.entities[0].temperature;
        return getDeviceData(date, devicevalue1, '');
      } else {
        date = widget.deviceValues[4].bodyWeight.entities[0].lastsyncdatetime;
        devicevalue1 = widget.deviceValues[4].bodyWeight.entities[0].weight;
        return getDeviceData(date, devicevalue1, '');
      }
    }
  }
